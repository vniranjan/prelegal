...
- Do no overengineer. Do no program defensively. Use exception managers only when needed.
- Identify root cause before fixing issues. Prove with evidence, then fix.
- Work incrementally in small steps. Validate each increment.
- Use latest library APIs.
- Use `uv` Python package manage. Always `uv run xxx` never `python3 xxx`, always `uv add xxx` never `pip install xxx`
